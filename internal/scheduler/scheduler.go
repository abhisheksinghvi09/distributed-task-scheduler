...
func (s *SchedulerServer) handleScheduleTask(w http.ResponseWriter, r *http.Request) {
	...
	scheduleTime, err := time.Parse(time.RFC3339, commandReq.ScheduledAt)
	if err != nil {
	...
	unixTimestamp := time.Unix(scheduleTime.Unix(), 0)
	...
	response := struct {
		...
		Command:     commandReq.Command,
		ScheduledAt: fmt.Sprintf("%d", unixTimestamp.Unix()),
		TaskId:      taskId,
	}
